...
import { ProgressiveMacroTokenizer, type MacroToken, progressiveMacroTokenStyles } from '../../editor/services/macro-tokenizer-progressive.ts';
                    ...
                    <div key={lineIndex} className="min-h-[1.25rem]">
                        {tokens.length === 0 ? (
                            ...
                            tokens.map((token, tokenIndex) => (
                                <span
                                    key={tokenIndex}
                                    className={progressiveMacroTokenStyles[token.type] || 'text-zinc-300'}
                                >
                                    {token.value}
                                </span>
                            ))
