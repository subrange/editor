...
export function Learning() {
  ...
  const [expandedCategories, setExpandedCategories] = useLocalStorageState<
    string[]
  >('learning-expanded-categories', []);
  const [expandedSubcategories, setExpandedSubcategories] =
    useLocalStorageState<string[]>('learning-expanded-subcategories', []);
  const [selectedItemId, setSelectedItemId] = useLocalStorageState<
    string | null
  >('learning-selected-item', null);
    ...
    if (selectedItemId && !learningStore.state.value.selectedItem) {
      ...
      for (const category of learningState.categories) {
        for (const subcategory of category.subcategories) {
          const foundItem = subcategory.items.find(
  ...
  const loadItem = (item: LearningItem) => {
    ...
    const { editorConfig, content, interpreterConfig, debuggerConfig, labels } =
      item;
    ...
    if (
      editorConfig.showMainEditor !== undefined &&
      showMainEditor !== String(editorConfig.showMainEditor)
    ) {
      localStorage.setItem(
        'showMainEditor',
        String(editorConfig.showMainEditor),
      );
    }

    if (
      editorConfig.showMacroEditor !== undefined &&
      showMacroEditor !== String(editorConfig.showMacroEditor)
    ) {
      localStorage.setItem(
        'showMacroEditor',
        String(editorConfig.showMacroEditor),
      );
    }
    ...
    if (editorConfig.mainEditorMode) {
      localStorage.setItem(
        'mainEditorMode',
        JSON.stringify(editorConfig.mainEditorMode),
      );
    }
                    ...
                    >
                      <div className="flex items-center gap-1.5 flex-1">
                        <div
                          className="transition-transform duration-200"
                          style={{
                            transform: expandedSubcategories.includes(
                              subcategory.id,
                            )
                              ? 'rotate(90deg)'
                              : 'rotate(0deg)',
          ...
          <div className="space-y-1">
            ...
            >
              ...
              <span className="font-medium">
                Complete IDE Features Overview
              </span>
            </button>
        ...
        <div className="mt-6 space-y-2">
          ...
          <div className="space-y-1">
            ...
            >
              ...
              <span className="font-medium">
                Ripple VM Assembly & Architecture
              </span>
            </button>
          </div>
        </div>
