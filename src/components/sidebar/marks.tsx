...
export function Marks() {
  ...
  const [activeEditor, setActiveEditor] = useState<'macro' | 'assembly'>(
    'macro',
  );
  ...
  useEffect(() => {
    const extractMarks = () => {
      ...
      const editorId =
        currentActiveEditor === 'assembly' || currentActiveEditor === 'macro'
          ? currentActiveEditor
          : activeEditor;
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-2">
          <h3 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">
            {activeEditor === 'macro' ? 'Macro' : 'Assembly'} Editor Marks (
            {marks.length})
          </h3>
