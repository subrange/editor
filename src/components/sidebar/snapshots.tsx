...
function saveSnapshots(snapshots: TapeSnapshot[]) {
  try {
    ...
    if (error instanceof DOMException && error.name === 'QuotaExceededError') {
      alert(
        'Failed to save snapshots: Storage quota exceeded. Please delete some old snapshots to free up space.',
      );
    } else {
      alert(
...
export function Snapshots() {
  const [snapshots, setSnapshots] = useState<TapeSnapshot[]>(() =>
    loadSnapshots(),
  );
  ...
  const createSnapshot = () => {
    ...
    try {
      ...
      if (serialized.length > 5 * 1024 * 1024) {
        // 5MB safety limit
        alert(
          `Snapshot is too large (${(serialized.length / 1024 / 1024).toFixed(2)}MB). The tape has ${usedLength.toLocaleString()} non-empty cells. Consider using a smaller tape or clearing unused cells.`,
        );
        return;
      }
      ...
      if (error instanceof RangeError) {
        alert(
          `Cannot create snapshot: The tape is too large (${state.tape.length.toLocaleString()} cells). The browser cannot allocate enough memory for this operation.`,
        );
      } else {
        alert(
