...
export function AssemblyAutocompleteWrapper({
  store,
  charWidth,
}: AssemblyAutocompleteWrapperProps) {
  ...
  const previousCursorRef = useRef<{ line: number; column: number }>({
    line: 0,
    column: 0,
  });
  ...
  useEffect(() => {
    ...
    if (wordMatch) {
      ...
      if (
        ...
        // Calculate position
        const x =
          LINE_PADDING_LEFT + (cursorPos.column - word.length) * charWidth;
