...
export function VirtualizedLine({
  ...
  const visibleTokens = useMemo(() => {
    ...
    const filtered = viewportTokenizer.filterTokensForViewport(
      tokens,
      lineText,
    );
    return filtered;
  ...
  if (!shouldVirtualize) {
    ...
    return (
      <>
        <div
          ref={containerRef}
          className={clsx('whitespace-pre pl-2 pr-4', {
            'bg-zinc-900':
              showDebug && isCurrentLine && isRunning && !hasBreakpoint,
                  ...
                  styles[token.type as keyof typeof styles] || '',
                  {
                    'cursor-pointer hover:underline':
                      ...
                      (((token.type === 'macro_invocation' ||
                        ...
                        isProgressiveMacro) ||
                        ((token.type === 'label' ||
                          token.type === 'label_ref') &&
                          isAssembly)),
                    'cursor-pointer hover:bg-zinc-800 hover:rounded':
                      isShiftKeyHeld &&
                      (token.type === 'macro_invocation' ||
                        token.type === 'hash_macro_invocation' ||
                        token.type === 'macro_name') &&
                      ...
                      'cursor-pointer hover:underline':
                        ...
                        (((token.type === 'macro_invocation' ||
                          ...
                          ((token.type === 'label' ||
                            token.type === 'label_ref') &&
                            isAssembly)),
                      'cursor-pointer hover:bg-zinc-800 hover:px-1 hover:rounded':
                        isShiftKeyHeld &&
                        (token.type === 'macro_invocation' ||
                          token.type === 'hash_macro_invocation' ||
                          token.type === 'macro_name') &&
  ...
  return (
    <>
      <div
        ref={containerRef}
        className={clsx('overflow-x-auto whitespace-pre', {
          'bg-zinc-900':
            showDebug && isCurrentLine && isRunning && !hasBreakpoint,
          'bg-red-950': showDebug && hasBreakpoint,
        })}
              ...
              if (token.type === 'truncation-indicator') {
                const position =
                  token.originalStart === 0
                    ? scrollLeft - charWidth
                    : scrollLeft + editorWidth;
                return (
                  <span
                    key={tokenIndex}
