...
export function MacroAutocomplete({
  ...
  const x = triggerPosition
    ? triggerPosition.column * charWidth + LINE_PADDING_LEFT
    : 0;
  const lineY = triggerPosition
    ? triggerPosition.line * CHAR_HEIGHT + LINE_PADDING_TOP
    : 0;
  ...
  const popupHeight = Math.min(
    280,
    filteredMacros.length * itemHeight + footerHeight,
  );
        ...
        {filteredMacros.map((macro, index) => (
          ...
          >
            <div className="flex items-center gap-2 w-full">
              <span
                className={prefix === '@' ? 'text-blue-400' : 'text-green-400'}
              >
                {prefix}
              </span>
