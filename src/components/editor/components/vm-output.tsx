...
export function VMOutput({ outputRef, isActive = true }: VMOutputProps) {
  ...
  return (
    <div
      ref={activeRef}
      className="flex flex-col h-full overflow-auto bg-zinc-950"
    >
      {/* Header */}
      <div className="sticky top-0 z-10 bg-zinc-900 border-b border-zinc-800">
        <div className="px-3 py-2 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <button
              onClick={() =>
                vmTerminalStore.updateConfig({ enabled: !enabled })
              }
              className={clsx(
              ...
              title={
                enabled ? 'VM output is syncing' : 'VM output sync is disabled'
              }
            >
              {enabled ? 'ON' : 'OFF'}
            </button>
            <div className="flex items-center gap-2 text-[11px] text-zinc-500 font-mono overflow-hidden">
              <span
                ...
                title={`Output cell: ${outCellIndex}`}
              >
                [{outCellIndex}]
              </span>
              ...
              <span
                className="whitespace-nowrap"
                title={`Flag cell: ${outFlagCellIndex}`}
              >
                [{outFlagCellIndex}]
              </span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer group pt-2">
                <input
                  ...
                  checked={clearOnRead}
                  onChange={(e) =>
                    vmTerminalStore.updateConfig({
                      clearOnRead: e.target.checked,
                    })
                  }
