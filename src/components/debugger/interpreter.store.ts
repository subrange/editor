...
import type {
  SourceMap,
  SourceMapEntry,
} from '../../services/macro-expander/source-map.ts';
...
class InterpreterStore {
      ...
      if (mainEditor) {
        ...
        this.editorSubscription = mainEditor.editorState.subscribe((s) => {
          if (JSON.stringify(s.lines) !== JSON.stringify(this.code)) {
            // Yep, I do not care about performance here
            this.reset();
            this.code = s.lines;
            this.buildLoopMap();
          }
  ...
  public stepToPosition(targetPosition: Position) {
    ...
    if (
      targetPosition.line === currentChar.line &&
      targetPosition.column === currentChar.column
    ) {
      return;
    }
  ...
  public toggleSourceBreakpoint(sourcePosition: Position) {
    ...
    const index = sourceBreakpoints.findIndex(
      (bp: Position) =>
        bp.line === sourcePosition.line && bp.column === sourcePosition.column,
    );
    ...
    if (expandedEntries.length === 0) {
      ...
      const lineEntries =
        (this.sourceMapLookup as any).sourceMap?.sourceToExpanded.get(
          lineKey,
        ) || [];
      ...
      if (lineEntries.length > 0) {
        ...
        console.warn(
          'No expanded positions found for source position:',
          sourcePosition,
        );
        console.warn(
          'This line may not contain executable code (e.g., comment or empty line)',
        );
    ...
    for (const entry of expandedEntries) {
      const entryLength =
        entry.expandedRange.end.column - entry.expandedRange.start.column;
      const fullLength =
        fullExpansionEntry.expandedRange.end.column -
        fullExpansionEntry.expandedRange.start.column;
    ...
    for (const expandedPos of expandedPositions) {
      if (
        breakpoints.some(
          (bp) =>
            bp.line === expandedPos.line && bp.column === expandedPos.column,
        )
      ) {
        hasBreakpoint = true;
        break;
    ...
    switch (char) {
      case '>':
        currentState.pointer =
          (currentState.pointer + 1) % currentState.tape.length;
        this.operationCount++;
        break;
      case '<':
        currentState.pointer =
          (currentState.pointer - 1 + currentState.tape.length) %
          currentState.tape.length;
        this.operationCount++;
        break;
      case '+':
        const increment = settingsStore.settings.getValue().weird?.doublePlus
          ? 2
          : 1;
        currentState.tape[currentState.pointer] =
          (currentState.tape[currentState.pointer] + increment) %
      ...
      case '[':
        if (currentState.tape[currentState.pointer] === 0) {
          ...
          if (matchingPos) {
            ...
            console.error(
              `No matching ] for [ at ${currentPos.line}:${currentPos.column}`,
            );
          }
        }
        this.operationCount++;
      ...
      case ']':
        ...
        break;
      case '.':
        currentState.output += String.fromCharCode(
          currentState.tape[currentState.pointer],
        );
        this.operationCount++;
        break;
      ...
      switch (char) {
        ...
        case '+':
          const increment = settingsStore.settings.getValue().weird?.doublePlus
            ...
            : 1;
          tape[pointer] =
            (tape[pointer] + increment) % this.cellSize.getValue();
          break;
        case '-':
          tape[pointer] =
            (tape[pointer] - 1 + this.cellSize.getValue()) %
            this.cellSize.getValue();
          break;
        case '[':
          if (tape[pointer] === 0) {
    ...
    while (this.state.getValue().isRunning && !this.state.getValue().isPaused) {
      ...
      for (let i = 0; i < BATCH_SIZE; i++) {
        if (!this.stepOptimized()) {
          ...
          console.log(
            `Program completed: ${totalSteps + batchSteps} instructions in ${totalTime}s`,
          );
          return;
        }
        batchSteps++;
      ...
      console.log(
        `Progress: ${totalSteps} instructions in ${elapsed}s (${Math.round(totalSteps / elapsed)} ops/sec)`,
      );
  ...
  private async runTurboFromCurrentPosition() {
    ...
    while (pc < ops.length) {
      ...
      switch (op.type) {
        case '>':
          pointer = (pointer + 1) % this.tapeSize.getValue();
          break;
        case '<':
          pointer =
            (pointer - 1 + this.tapeSize.getValue()) % this.tapeSize.getValue();
          break;
        case '+': {
          const increment = settingsStore.settings.getValue().weird?.doublePlus
            ? 2
  ...
  public async runTurbo() {
    ...
    console.log(
      `Compiled ${ops.length} operations. Starting turbo execution...`,
    );
    ...
    while (pc < ops.length) {
      ...
      switch (op.type) {
        ...
        case '$': {
          ...
          console.log(
            'Exiting turbo mode due to breakpoint.rs. Use step or resume to continue.',
          );
          return;
        }
      }
      ...
      if (opsExecuted % UPDATE_INTERVAL === 0) {
        const elapsed = (performance.now() - startTime) / 1000;
        console.log(
          `Turbo progress: ${opsExecuted} ops in ${elapsed}s (${Math.round(opsExecuted / elapsed)} ops/sec)`,
        );
    ...
    console.log(
      `Turbo execution completed: ${opsExecuted} operations in ${totalTime}s (${Math.round(opsExecuted / totalTime)} ops/sec)`,
    );
  }
  ...
  public stop() {
    ...
    this.state.next({
      ...
      isStopped: true,
      lastExecutionTime:
        executionTime || this.state.getValue().lastExecutionTime,
      lastOperationCount:
        this.operationCount || this.state.getValue().lastOperationCount,
    });
  ...
  public provideInput(char: string) {
    ...
    currentState.tape[currentState.pointer] =
      asciiValue % this.cellSize.getValue();
    ...
    console.log(
      `Input received: '${char}' (ASCII ${asciiValue}) placed at position ${currentState.pointer}`,
    );
  ...
  private updateSourcePosition() {
    ...
    if (entry) {
      ...
      context.forEach((e, i) => {
        console.log(
          `  ${i}: ${e.macroName} ${e.parameterValues ? JSON.stringify(e.parameterValues) : ''}`,
        );
      });
      ...
      // Debug logging
      console.log(
        `Source position updated: Expanded [${currentPos.line}:${currentPos.column}] -> Source [${sourcePos.line}:${sourcePos.column}] (${entry.macroName || 'direct'})`,
      );
    } else {
      this.currentSourceChar.next(null);
      const currentState = this.state.getValue();
      ...
      // Debug logging
      console.log(
        `No source position for expanded position [${currentPos.line}:${currentPos.column}]`,
      );
    }
  }
}
