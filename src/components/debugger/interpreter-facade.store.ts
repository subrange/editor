...
import {
  interpreterStore as jsInterpreter,
  type TapeSnapshot,
} from './interpreter.store.ts';
...
class InterpreterFacade implements InterpreterInterface {
  ...
  public currentChar = new BehaviorSubject<Position>(
    jsInterpreter.currentChar.getValue(),
  );
  public currentSourceChar = new BehaviorSubject<Position | null>(
    jsInterpreter.currentSourceChar?.getValue() || null,
  );
  public tapeSize = new BehaviorSubject<number>(
    jsInterpreter.tapeSize.getValue(),
  );
  public cellSize = new BehaviorSubject<number>(
    jsInterpreter.cellSize.getValue(),
  );
  public laneCount = new BehaviorSubject<number>(
    jsInterpreter.laneCount.getValue(),
  );
  ...
  private setupProxying() {
    ...
    this.subscriptions.push(
      ...
      this.currentInterpreter.currentChar.subscribe((value) =>
        this.currentChar.next(value),
      ),
      this.currentInterpreter.tapeSize.subscribe((value) =>
        this.tapeSize.next(value),
      ),
      this.currentInterpreter.cellSize.subscribe((value) =>
        this.cellSize.next(value),
      ),
      this.currentInterpreter.laneCount.subscribe((value) =>
        this.laneCount.next(value),
      ),
    );
    ...
    if (this.currentInterpreter.currentSourceChar) {
      this.subscriptions.push(
        this.currentInterpreter.currentSourceChar.subscribe((value) =>
          this.currentSourceChar.next(value),
        ),
      );
    }
  }
  ...
  private async switchToWorker() {
    ...
    // Copy breakpoints
    if (
      currentState.breakpoints.length > 0 ||
      (currentState.sourceBreakpoints?.length || 0) > 0
    ) {
    ...
    if (
      this.pendingVMOutputConfig &&
      'setVMOutputConfig' in this.workerInterpreter
    ) {
      (this.workerInterpreter as any).setVMOutputConfig(
        this.pendingVMOutputConfig,
      );
    }

    if (
      this.pendingVMOutputCallback &&
      'setVMOutputCallback' in this.workerInterpreter
    ) {
      (this.workerInterpreter as any).setVMOutputCallback(
        this.pendingVMOutputCallback,
      );
    }
  }
  ...
  async setVMOutputConfig(config: {
    outCellIndex: number;
    outFlagCellIndex: number;
    clearOnRead?: boolean;
    sparseCellPattern?: any;
