...
export class SourceMapBuilder {
  ...
  addMapping(entry: SourceMapEntry): void {
    ...
    const expandedLineEntries = this.expandedToSource.get(expandedLineKey) || [];
...
export class SourceMapLookup {
  ...
  getSourcePosition(expandedLine: number, expandedColumn: number): SourceMapEntry | null {
    // First try exact match
    const exactKey = `${expandedLine}:${expandedColumn}`;
    const exactEntries = this.sourceMap.expandedToSource.get(exactKey);
    console.log(`getSourcePosition: Looking for [${expandedLine}:${expandedColumn}], found ${exactEntries?.length || 0} exact entries`);
    
    if (exactEntries && exactEntries.length > 0) {
      // Debug: Show all entries at this position
      exactEntries.forEach((e, i) => {
        console.log(`  Entry ${i}: ${e.macroName} depth=${e.expansionDepth} hasCallStack=${!!e.macroCallStack}`);
      });
      ...
      console.log(`  Returning: ${result.macroName} depth=${result.expansionDepth}`);
      return result;
    }
    ...
    for (const entry of this.sourceMap.entries) {
      if (this.positionInRange(expandedLine, expandedColumn, entry.expandedRange)) {
        const distance = this.distanceToStart(expandedLine, expandedColumn, entry.expandedRange);
  ...
  getExpandedPositions(sourceLine: number, sourceColumn: number): SourceMapEntry[] {
    // First try exact match
  ...
  getMacroContext(expandedLine: number, expandedColumn: number): SourceMapEntry[] {
    // Get the full macro expansion stack at a position
    const entry = this.getSourcePosition(expandedLine, expandedColumn);
    
    console.log('getMacroContext: entry =', entry ? {
    ...
    if (entry && entry.macroCallStack && entry.macroCallStack.length > 0) {
      ...
      console.log('getMacroContext: returning', result.length, 'entries from call stack');
      return result;
    }
