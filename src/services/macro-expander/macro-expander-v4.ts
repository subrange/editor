...
export class MacroExpanderV4 implements MacroExpander {
  ...
  private expandBrainfuckCommand(
    node: BrainfuckCommandNode,
    ...
    if (macro.parameters && node.arguments) {
      for (let i = 0; i < macro.parameters.length; i++) {
        if (i < node.arguments.length) {
          parameters[macro.parameters[i]] = this.expandExpressionToString(
            node.arguments[i],
            context,
          );
  ...
  private expandIf(node: BuiltinFunctionNode, context: ExpansionContext): void {
    ...
    const conditionStr = this.expandExpressionToString(
      node.arguments[0],
      context,
    );
  ...
  private expandFor(
    ...
    if (arrayNode.type === 'ArrayLiteral') {
      ...
      context.macroCallStack.length > 0
    ) {
  ...
  private expandBodyWithSubstitutions(
    ...
    for (const node of body) {
      ...
      if (
        lastNodeEndLine !== undefined &&
        node.position &&
        node.position.line > lastNodeEndLine
      ) {
        ...
        if (
          ...
          const substituteInExpression = (
            ...
            if (expr.type === 'Identifier') {
              ...
              if (
                currentMacro.parameterNodes &&
                currentMacro.parameterNodes[idNode.name]
              ) {
          ...
          const substitutedArgs = builtinNode.arguments.map(
  ...
  private expandMetaVariables(text: string, context: ExpansionContext): string {
    ...
    text = text.replace(
      /__INVOC_COUNT__/g,
      context.invocationCounter.toString(),
    );
    ...
    text = text.replace(
      /__LABEL__(?:\(([^)]+)\)|(\w+))/g,
      (match, parenPrefix, simplePrefix) => {
    ...
    if (context.macroCallStack.length > 1) {
      const parentMacro =
        context.macroCallStack[context.macroCallStack.length - 2];
