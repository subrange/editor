import init, { WasmAssembler, WasmLinker } from '../../ripple-asm/pkg/ripple_asm.js';
...
export class RippleAssembler {
    ...
    async assemble(source: string): Promise<AssemblerResult> {
        ...
        try {
            ...
            const unresolvedRefs = objectFile.unresolved_references || objectFile.unresolvedReferences;
            const hasUnresolved = unresolvedRefs && (
                unresolvedRefs instanceof Map ? unresolvedRefs.size > 0 : Object.keys(unresolvedRefs).length > 0
            );
            
            let finalResult;
            if (hasUnresolved) {
                const numRefs = unresolvedRefs instanceof Map ? unresolvedRefs.size : Object.keys(unresolvedRefs).length;
                console.log('Running linker to resolve', numRefs, 'references:', unresolvedRefs);
