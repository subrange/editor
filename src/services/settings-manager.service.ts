    ...
    try {
      const allSettings: AllSettings = {
        ...
        settings: {
          ...
          // Tape labels
          tapeLabels: tapeLabelsStore.labels?.value || {
            lanes: {},
            columns: {},
            cells: {},
          },
  ...
  importAllSettings(jsonString: string): void {
    try {
      ...
      if (!this.isVersionCompatible(imported.version)) {
        throw new Error(
          `Incompatible settings version: ${imported.version}. Expected ${this.SETTINGS_VERSION}`,
        );
      }

      // Import core settings
      if (imported.settings.macro) {
        settingsStore.setMacroStripComments(
          imported.settings.macro.stripComments,
        );
        settingsStore.setMacroCollapseEmptyLines(
          imported.settings.macro.collapseEmptyLines,
        );
      ...
      if (imported.settings.tapeLabels) {
        ...
        // Import lane labels
        Object.entries(imported.settings.tapeLabels.lanes).forEach(
          ([lane, label]) => {
        ...
        Object.entries(imported.settings.tapeLabels.columns).forEach(
          ([col, label]) => {
        ...
        // Import cell labels
        Object.entries(imported.settings.tapeLabels.cells).forEach(
          ([index, label]) => {
