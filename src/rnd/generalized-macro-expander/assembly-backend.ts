...
import type { ContentNode, TextNode, BrainfuckCommandNode } from '../../services/macro-expander/macro-parser.ts';
...
export class AssemblyBackend implements MacroBackend {
  ...
  validateNode(node: any): ValidationError[] {
    ...
    if (node.type === 'BrainfuckCommand') {
      errors.push({
        type: 'warning',
        message: 'Brainfuck commands in assembly context will be treated as comments',
