...
pub fn emit_complete_program(
    ...
    // Check if we have _init_globals
    let has_init_globals = instructions.iter().any(|inst| {
        matches!(inst, AsmInst::Label(label) if label == "_init_globals")
    });
    
    // Add program entry point if needed
    if has_main {
        output.push_str("; Program entry point\n");
        output.push_str("_start:\n");
        output.push_str("    ; Initialize stack pointer\n");
        output.push_str("    LI R13, 0       ; Stack bank = 0\n");
        output.push_str("    LI R14, 1000   ; Stack pointer starts at 1000\n");
        output.push_str("    LI R15, 1000   ; Frame pointer starts at 1000\n");
        if has_init_globals {
            output.push_str("    ; Initialize globals\n");
            output.push_str("    CALL _init_globals\n");
        }
