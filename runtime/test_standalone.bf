//Pefectly Engineered Nonstandard Instruction Set v1.0
...
#define PROGRAM {
  ...
  @prg(
    ...
    @cmd(@OP_STOR  , @R6 , @R13, 8)
    @cmd(@OP_BEQ   , @R0 , @R0 , 1)
    @cmd(@OP_ADDI  , @R6 , @R15, 1)
    @cmd(@OP_LOAD  , @R5 , @R13, @R6)
    @cmd(@OP_SLTU  , @R7 , @R5 , @R5)
    @cmd(@OP_BNE   , @R7 , @R0 , 2)
    @cmd(@OP_BEQ   , @R0 , @R0 , 13)
    @cmd(@OP_ADDI  , @R6 , @R15, 1)
    @cmd(@OP_LOAD  , @R5 , @R13, @R6)
    ...
    @cmd(@OP_LI    , @R4 , 1   , 0)
    @cmd(@OP_ADD   , @R7 , @R5 , @R4)
    @cmd(@OP_ADDI  , @R8 , @R15, 1)
    @cmd(@OP_STOR  , @R7 , @R13, 10)
    @cmd(@OP_BEQ   , @R0 , @R0 , 65520)
    @cmd(@OP_ADD   , @R14, @R15, @R0)
    @cmd(@OP_ADDI  , @R14, @R14, 65535)
    @cmd(@OP_LOAD  , @R15, @R13, @R14)
    @cmd(@OP_ADDI  , @R14, @R14, 65535)
    ...
    @cmd(@OP_LOAD  , @R5 , @R13, @R6)
    @cmd(@OP_ADD   , @R7 , @R4 , @R5)
    @cmd(@OP_LOAD  , @R8 , @R0 , @R7)
    @cmd(@OP_ADDI  , @R10, @R15, 1)
    @cmd(@OP_LOAD  , @R9 , @R13, @R10)
    @cmd(@OP_ADD   , @R11, @R3 , @R9)
    @cmd(@OP_STOR  , @R8 , @R0 , 13)
    @cmd(@OP_BEQ   , @R0 , @R0 , 1)
    @cmd(@OP_ADDI  , @R6 , @R15, 1)
    @cmd(@OP_LOAD  , @R5 , @R13, @R6)
    @cmd(@OP_LI    , @R4 , 1   , 0)
...
#define lt {
  ...
  @prevword @clear @prevword < @prevword @clear @nextword
...
#define alu_div {
  ...
  @lane(@L_SA,
    ...
    @nextword @clear
    @nextword @clear
    @nextword @clear // mod here
    @nextword
    @move_to(@prevwords(2) >, < @nextwords(2)) <
  )
}
...
#define alu_slt {
  ...
  @lane(@L_SA,
    @nextwords(8)
    {repeat(18, @clear @nextword)}
    >
    {repeat(18, @clear @prevword)}
    <
    @prevwords(8)
  )
...
#define ins_jal {
  ...
  // Copy PC to RA // Here is the bug! We need to put +1 here!
...
#define mod  @ophandler(@two_registers_op(@alu_mod))
#define div  @ophandler(@two_registers_op(@alu_div))
...
#define modi  @ophandler(@two_registers_op(@alu_mod))
#define divi  @ophandler(@two_registers_op(@alu_div))
