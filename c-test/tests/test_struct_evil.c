// Evil struct test - comprehensive and boundary-pushing
void putchar(int c);
...
int main() {
    ...
    // Test 11: Complex expression with struct members
    local.a = local.nested.x + local.nested.y;  // 99 + 30 = 129
    if (local.a == 129) {
        putchar('B');
    } else {
        putchar('N');
    ...
    // Test 12: Struct assignment through various paths
    global_evil.ptr = &local.a;
    *global_evil.ptr = 555;
    if (local.a == 555) {
        putchar('C');
    } else {
